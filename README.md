...
- **NEPA screening surfaces on PNNL's nepa-mcp** (v1.29, 2026-08) — a shared screening engine (`scripts/nepa_screening.py`, ten normalizers over the pinned `nepa-mcp==0.1.1` federal-GIS toolkit) powers three products: the Army-Janus installation screens (Microreactors tab), per-plant "Permitting screen" chips in the Coal Reinvestment drawer (18 plants × 7 sources), and the **Hanford tab** — an end-to-end dossier of one DOE site: 9 named land units, each with cited ground truth, a ten-source environmental screen (USFWS *and* NOAA Fisheries lenses, tribal geography, historic properties, protected areas, USACE district, flood, biodiversity, EPA NEPAssist), permitting/licensing pathways (NEPA, CERCLA/TPA, NRC, WA EFSEC, ESA §7, EFH, NHPA §106, CWA 404), 14-layer GeoJSON map packages incl. EPA EIS boundaries, and honest opportunity fits — including "this land is off the table." Screening evidence, never an agency determination.
