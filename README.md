...
- **Maritime Siting lenses** (2026-08) — two more 0–100 scores for sites near a coastal or Great Lakes port: floating/offshore nuclear (weighted on proximity + capability of a curated set of 16 major US heavy-shipbuilding yards, since every real floating-reactor program — Akademik Lomonosov, ThorCon, Core Power — assembles at a shipyard and tows to site) and coastal on-site generation/data centers (grid-dominant, with port access as a secondary heavy-equipment-delivery signal). Joins all 46,759 sites against 108 BTS/USACE Principal Ports and the curated shipyard list; rendered as the Maritime Siting tab plus ⚓/⚒ map markers.
- **Nickel Refining lenses + the first water layer** (2026-09) — two more 0–100 scores for a nickel hydrometallurgical refinery, the one nickel facility type that plausibly fits a US brownfield. A sulfide smelter is deliberately not scored: none operates in the US, EPA never wrote a nickel-smelting emissions standard, and the continuous furnace load pairs with a captive mine and captive generation. The two lenses follow the opposite siting logic the two live US projects actually published — Electra weighting deep-water port access for imported intermediate, Westwin weighting rail after building in landlocked Lawton, Oklahoma. Land is a 300-acre threshold rather than a ranked factor, and it is tri-state, because EPA publishes no acreage at all for its ~36,000 brownfield properties. Shipped alongside the project's first quantitative water signal: active USGS streamgages with mean annual discharge, joined to all 46,759 sites. That flow figure is a long-run average, not a permittable low flow, so it screens a candidate and never clears one.
