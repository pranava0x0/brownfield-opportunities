...
Static dashboard of EPA Superfund + ACRES brownfield + USACE FUDS + DOD BRAC sites — evidence browser, map and table with filters, site-level detail, infrastructure evidence categories, ECHO enforcement signals, AI-generated narrative summaries, CSV export, and shareable URL state. Designed to live on GitHub Pages and refresh via a one-command CLI.
...
- **USACE FUDS** — 8,821 Formerly Used Defense Sites with layer-1/layer-4 join; ~3k carry polygon-derived acreage. Owner labels normalized from raw USACE codes.
...
- **Approach C evidence pilots** — one Franklin County site carries a USGS network-linked reach/HUC and MassBroadband 123 regional-network context; Portsmouth carries named DOE grid equipment. These prove the source/scope/unknown contract without claiming parcel service, water supply, spare capacity, or rights.

**Frontend** —

Explore opens first, with alphabetical sites and separate Grid, Fiber, Water, Land and Logistics findings. Confidence describes each finding, not suitability. No composite totals or hidden numerical ranking. Map markers, optional catalogs and detailed evidence load on demand. [Data guide](docs/infrastructure-data-guide.md) · [Implementation assessment](docs/specs/spec-infrastructure-scoring-audit-2026-09-12.md).

- **Microreactors** — vendor designs, named commitments, and a site evidence screen. Grid, demand and regulatory pathway are separate findings. Missing grid observations never establish isolation or customer demand.
- **Maritime Siting** — port and shipyard context with separate grid, water, land and logistics findings. Proximity does not establish berth access, fabrication capacity or a tow route.
- **Nickel Refining** — imported-feed and domestic-feed views with explicit reported-land thresholds (300 acres for refining; 100 for recycling; unknown can be included). USGS gages provide stream context; intake feasibility, reliable supply and allocation remain unassessed.
...
- Five evidence-coverage metrics for the filtered population: sites, reported acreage, mapped grid, gage context and fiber evidence. Mobile metrics start collapsed.
...
- Mobile category cards show every primary finding without horizontal panning. Details open in a bottom sheet.
...
- `docs/data/site-evidence-pilots.json` — small regional/network evidence pilots; generated by `scripts/build_site_evidence_pilots.py`.
...
### Infrastructure evidence refresh

`python3 scripts/rebuild_infra_from_cache.py` recalculates spatial matches from retained raw snapshots, one layer at a time. It preserves source dates and records unknown snapshot dates explicitly; its compilation date does not certify current infrastructure. A successful rebuild must retain the inventory and pass schema/count checks before publication.

After producer coordinates change, rebuild affected joins. `scripts/build_evidence_invalidations.py --before-dir <retained-producer-snapshots>` records changed coordinates and withholds geographic fields from joins not rebuilt. Review the `--rebuilt` list against actual completed runs. The browser applies this manifest after late loads, in category findings, details and CSV exports. Sub-meter serialization changes use an explicit one-meter tolerance.

Legacy `*-score.js` files remain as historical source for earlier releases; the application no longer loads them. Current runtime rules are `docs/evidence-categories.js` and planning thresholds in `docs/siting-context.js`.
