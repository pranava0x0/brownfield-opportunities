...
- **Map legend.** Custom `L.control` in `bottomright` showing the four NPL status colors + a "marker size ∝ acreage (log)" footnote. Colors live in `colorForStatus()` (app.js) and the legend's `STATUS_LEGEND` array — keep them in sync with the `--status-*` CSS variables and the `.pill[data-status="X"]` rules. Single source of truth would be nice; flagged in backlog.
