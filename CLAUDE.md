...
- **Status colors are CSS-var-driven.** Single source of truth: `--status-final`, `--status-deleted`, `--status-proposed`, `--status-other`, `--status-acres` defined on `:root` and overridden under `[data-theme="dark"]`. JS reads via `getComputedStyle(html).getPropertyValue(varName)`. **Don't** hard-code marker colors in `colorForRecord()` or duplicate them in CSS — the legend, pill rules, and JS all read the same vars. On theme swap, `refreshMarkerColors()` re-styles every marker.
...
- **URL state sharing.** `?site=<ID>` (legacy `?epa_id=` accepted), `?q=`, `?state=`, `?status=F,P`, `?program=superfund,brownfield`, `?min_ac=<log10>`. Writes are debounced 200ms via `history.replaceState` so slider drags don't spam history. Programs only encoded when not the default (Superfund only).
...
- **Tile provider.** CARTO `light_all` (pre-rendered light) — match it to the UI theme. **Do not** layer a CSS `filter: brightness/contrast` on `.leaflet-tile-pane` to retheme bright tiles — the filter recomposites every tile during pan/zoom and tanks mobile performance. If swapping providers, pick one whose tiles already match the UI theme. Tile layer also uses `bounds: US_BOUNDS` + `noWrap: true` to skip fetches outside the US.
- **US-only constraint.** `maxBounds` (≈ `[15, -180]` to `[72, -60]`, covering AK Aleutians + PR), `maxBoundsViscosity: 1.0`, and `minZoom: 3` together prevent the user from panning or zooming to non-US areas. Don't loosen these without a reason — the dashboard is a US Superfund tool.
- **Map legend.** Custom `L.control` in `bottomright` showing the four NPL status colors + the brownfield series + a "marker size ∝ acreage (log)" footnote. Colors come from CSS vars via `cssColor()`. Theme swap re-renders the legend (`document.querySelectorAll('.legend').forEach(n => n.remove()); addLegend();`) so the dot colors update.
