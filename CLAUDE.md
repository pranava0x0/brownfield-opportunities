...
- **Color tokens are CSS-var-driven.** Two palettes live on `:root` (with `[data-theme="dark"]` overrides):
  ...
  - `--program-superfund` / `--program-brownfield` / `--program-fuds` / `--program-brac` — drive **map markers and the legend**. Markers are colored by program (one dot per program), not by NPL status. JS reads via `getComputedStyle(html).getPropertyValue(varName)`. `colorForRecord()` reads `PROGRAM_COLOR_BY_PROGRAM`. **Don't** hard-code marker colors or duplicate the values in CSS — the legend, pill rules, and JS all read the same vars. On theme swap, `refreshMarkerColors()` and `rerenderLegend()` re-paint everything. FUDS = purple (#7b3fa0 light / #b47ad8 dark), BRAC = orange (#d47a1a light / #e6a040 dark).
...
- **Tests are the safety net.** 143 tests total: pytest unit tests cover normalize/envelope/fetch/dedupe/merge/diff/schema for all connectors (superfund-npl, epa-acres, dod-fuds, dod-brac, epa-redev). 26 Playwright e2e tests cover page load, tab switch, marker click, table click, Esc close, search filtering, legend render, all four programs loading, NPL status checkboxes, state dropdown, acreage slider, pagination, DOM size cap, accessibility. CI (`.github/workflows/test.yml`) runs both on every PR. Don't ship a connector without tests — the schema's `extra="forbid"` will catch drift but tests catch logic regressions.
