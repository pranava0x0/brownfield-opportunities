...
- **Tests are the safety net.** 152 tests total: pytest unit tests cover normalize/envelope/fetch/dedupe/merge/diff/schema for all connectors (superfund-npl, epa-acres, dod-fuds, dod-brac, epa-redev). 35 Playwright e2e tests cover page load, tab switch, marker click, table click, Esc close, search filtering, legend render, all four programs loading, NPL status checkboxes, state dropdown, acreage slider, pagination, DOM size cap, accessibility, plus v1.8: hero/KPI/footer rendering, meta text per-program counts, filter chip visibility, prettifier output for cities/counties/addresses, detail-panel program stripe, sort glyph, and the `[hidden]` trap regression. CI (`.github/workflows/test.yml`) runs both on every PR. Don't ship a connector without tests — the schema's `extra="forbid"` will catch drift but tests catch logic regressions.
