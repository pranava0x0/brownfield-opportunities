...
- **[med] Low-flow (7Q10), so the water number can clear a site instead of only screening one.** `water_flow_cfs` is a long-run annual mean and every surface says so, because a withdrawal permit is written against drought-of-record flow that is routinely an order of magnitude lower. USGS publishes low-flow statistics per gage but not through the same bulk RDB path; finding the endpoint would turn a screening signal into a permitting-grade one.
- **[low] Workforce by industry, not headcount.** Electra's published criterion is "workforce availability in established chemical processing and manufacturing regions", but the corpus carries only `county_labor_force`, so both nickel lenses use a coarse proxy and weight it lowest (8-10 of 100). County employment in NAICS 325 (chemicals) and 331 (primary metal) from Census CBP would make the term mean what the citation says. Needs `CENSUS_API_KEY` — same unlock as the Spec 02 item above.
- **[low] The `#` rank column is 200px wide in every `.micro-table`.** Measured 2026-09-08 at tablet width: Rankings, Maritime and Nickel all give the rank column 200px against a 148px Site column, so every ranked table opens with a wide empty gutter. It is consistent across tabs, which is why it was left alone rather than fixed in one of them — the fix belongs in the shared `.micro-table` rule, and it should be checked against all of them at once.
