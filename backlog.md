...
- **[high] Continue docs backfill (193 eligible sites remaining).** Prefer resumable merge mode: `python3 refresh.py --source epa-superfund-docs --missing-only --docs-limit 50`; if the next run keeps rechecking known no-doc misses, use a small bounded skip window such as `--docs-skip 34 --docs-limit 10` and verify the count advances. The 2026-05-23/26 batches showed a bimodal pattern: early records were mostly cached "no docs / no SF_SITE_ID" misses, then later records were document-rich and slow because EPA's `semspub` cachejson endpoint took ~60s per collection. Keep future batches bounded at 25–50 unless running overnight.
