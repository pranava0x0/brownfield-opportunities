...
## Approach C after evidence categories — 2026-09-12

- **High:** Basin-connected water pilot: waterbody/flowline/catchment identity, source-matched low-flow statistic, record period, project demand and explicit intake/allocation unknowns. Extend issues #29/#30 rather than duplicate them.
- **High:** Regional enterprise fiber pilot with published coverage, route lifecycle and parcel service evidence. FCC availability and proposed grant routes remain distinct context.
- **High:** Recompute invalidated geographic joins for the 25 materially moved producer points; manifest remains authoritative until each dependency is rebuilt and verified.
- **High:** Migrate USGS legacy Water Services connector before announced Q1 2027 retirement; preserve statistic/period contracts and cache replay.
- **Medium:** Utility-specific capacity/connection evidence keyed to the actual asset and proposed parcel. Do not reuse retired plant MW as capacity rights.
- **Medium:** Partition or precompute compact evidence payloads if measured mobile first-insight time remains too high. Current improvements defer unused datasets and markers; nationwide core joins still have a substantial decoded footprint.

The detailed C prerequisites and changed assumptions are in `docs/specs/spec-infrastructure-scoring-audit-2026-09-12.md`. No national waterbody/fiber expansion was implemented in Approach B.
...
- **[med] `connectors/port_proximity.py`'s shipyard search doesn't reach the full advertised 150-mile radius everywhere.** `PointIndex.nearest_with_attr()`'s default 8-ring / 0.25°-cell search caps effective east-west reach at ~90-110 mi across much of CONUS (longitude compression at latitude), short of the connector's own `MAX_DISTANCE_MI_SHIPYARD = 150`. Codex round 3 (PR #26) found 602 sites with a yard inside 150 mi that got no `shipyard_mi` at all, plus 130 assigned a farther yard than the true nearest because the real nearest fell outside the search rings. Fix: pass a larger `max_rings` to the shipyard `nearest_with_attr()` call (16 shipyards nationwide is cheap enough for a full search), or special-case a wider ring count for this specific small-catalog index. Doesn't affect the port search (`MAX_DISTANCE_MI_PORT = 75` sits comfortably inside the default reach).
- **[low] Shipyard evidence link points to a generic MARAD page, not the matched yard's own citation.** `docs/provenance.js`'s `shipyard_mi` registry entry links to `maritime.dot.gov/data-reports` for every site, but each shipyard in `docs/data/shipyards.json` carries its own `source_url` (Wikipedia/company/CLUI page) that gets discarded by the connector's join. Codex round 3 (PR #26) flagged this — the fix needs either the connector to carry the matched yard's `source_url` through to `port-proximity.json`, or `buildEvidence()` to look it up from the shipyard overlay directly.
- **[low] `port-proximity.json` isn't in `scripts/validate_against_sources.py::ENRICHMENT_FOR_EVIDENCE`.** Same class of gap the codebase's own provenance-drift lesson describes (CLAUDE.md, 2026-08-09): `buildEvidence()` only emits a row for a field the sampled site actually carries, so `--only evidence` silently never exercises `port_mi`/`shipyard_mi` until the merge roster includes this file — and reports success while doing so. Codex round 3 (PR #26) flagged this; one-line fix, low urgency since `validate_data.py` (the CI-gating validator) already covers the join's structural correctness.
...
- **[med] Generic `build_nepa_screens.py --catalog` CLI (Spec 10 R1).** One command to screen ANY catalog JSON (id/name/lat/lon fields as flags) on the shared `nepa_screening.py` engine — kills the per-product script for one-off questions. Then run it over `nuclear-civilian-sites.json` (29 promising-tier) + `federal-clean-energy.json` (10) + `reference-campuses.json` (10) and render coal-style "Permitting screen" chips in their popups/drawers (Spec 10 R2).
- **[high] Hanford parcel polygons (Spec 11 H1).** Replace representative points with real boundaries where public: monument boundary (USFWS), MAPR park unit (NPS), NPL operable-unit polygons (EPA GeoPlatform site-boundary layer — full-coverage per data-source-research.md), EA-1915 conveyance parcels. The `coord_note` disclaimers exist because v1 is points.
- **[med] Second federal-site dossier (Spec 11 H3).** Repeat the Hanford pattern for INL (or Oak Ridge/ETTP) — the four Jul-2025 AI-DC flagships are already curated in `federal-clean-energy.json`. Two sites before genericizing; extract only what actually repeats.
- **[med] DOE NEPA-action tracker per Hanford parcel (Spec 11 H2).** Poll DOE's NEPA reading room for new Hanford CX/EA/EIS actions (CX-034321 was found by hand) and stamp `nepa_review_document_url` per parcel so `availability` stays evidence-backed between audits.
- **[high] Free credential unlocks: `CENSUS_API_KEY` + `EPA_AQS_EMAIL`/`EPA_AQS_API_KEY`.** Two free registrations unblock Spec 02's measured-workforce validation path and Spec 12's air-quality/NAAQS nonattainment gate. Keys live in env only; absence stays visibly unavailable.
- **[med] NAAQS nonattainment bulk connector (Spec 12 Tier B).** EPA Green Book nonattainment-area polygons on `PolygonIndex` (opportunity-zone shape) → `nonattainment_pollutants` per site. Display flag first; any score term needs its own weight-sum revision.
- **[low] Assert registry ⊆ roster in validate_data.py.** A one-line guard (`OVERLAY_SCHEMA_FILES.keys() ⊆ OVERLAY_FILES`, same for CURATED_PROVENANCE_FILES) makes the 2026-08-24 half-registration trap structurally impossible.
- **[med] Run `check_writing.py` in CI, not only in `pr_gate.sh`.** The linter gates locally today. A `.github/workflows/test.yml` job (`python3 scripts/check_writing.py` + `--commits origin/main..HEAD`) would catch banned phrasing on PRs opened without running the local gate. Cheap — no network, ~0.3 s.
- **[low] Triage the 18 remaining `citation-meta` WARNs in user-visible copy.** `python3 scripts/check_writing.py --only citation-meta`. Each is a judgement call: either the sentence should just carry the link, or the word is doing real work and the phrase belongs in `ALLOW_PHRASES` with a reason. Advisory-only today, so nothing is blocked.
- **[low] Two slop shapes `check_writing.py` cannot catch.** Bland-but-unpatterned prose ("Each land unit is assessed against the same four facility types") and one-off awkward jargon ("it binds through EFH, not Section 7") both survived the linter — neither is regex-detectable. If this keeps recurring, the lever is a review checklist or an LLM pass, not more regexes.
- **[high] Maine parcel acreage — an acreage-only state the registry cannot express.** MEGIS's statewide `Maine_Parcels_Organized_Towns` layer (708,382 polygons, no token, probed 2026-09-08 — data-source-research.md §34.5) has **no owner field**, so it fails `connectors/parcel_owner.py`'s registry contract, but its `Shape__Area` is in real metres (wkid 26919) and converts cleanly to acres. Maine is the mirror of the Iowa owner-only exception: an **acreage-only** state, which the registry has no path for today. This matters more than it sounds — acreage is present on only 75 of 898 Maine sites and 153 of 3,571 Michigan sites, and the Nickel Refining tab's 300-acre land test therefore reads "Unknown" for nearly every row in both states. Adding an acreage-only path (owner_field optional, mirroring how `owner_only_ok` works in reverse) would fill Maine. Michigan stays blocked — §19, no statewide layer at all.
- **[med] Distance to water, as distinct from distance to a gage.** `water_gage_mi` locates the flow figure, not the shoreline: gages sit tens of miles apart on a big river, so a site on the bank can read as several miles out, and a Great Lakes site with no gage at all reads as nothing in range. NHD small-scale waterbodies ≥10 km² (3,064 features) plus "Area - Small Scale" wide rivers (17,020) is ~20k polygons — cheap to index and it would separate "no water" from "no gage". §34.2 has the counts.
- **[med] Source-matched low-flow statistics for the water assessment.** `water_flow_cfs` is a long-run annual mean and every surface says so, because a withdrawal permit is written against drought-of-record flow that is routinely an order of magnitude lower. USGS publishes low-flow statistics per gage but not through the same bulk RDB path; an endpoint alone cannot establish supply or permitting readiness. Join the correct water source, period, intake, allocation and project demand before interpreting it.
- **[low] Workforce by industry, not headcount.** Electra's published criterion is "workforce availability in established chemical processing and manufacturing regions", but the corpus carries only `county_labor_force`, so workforce remains coarse context; Approach B removed numeric weighting. County employment in NAICS 325 (chemicals) and 331 (primary metal) from Census CBP would make the term mean what the citation says. Needs `CENSUS_API_KEY` — same unlock as the Spec 02 item above.
- **[resolved 2026-09-12] Removed the numeric rank column.** Previously the `#` column was 200px wide in every `.micro-table`. Measured 2026-09-08 at tablet width: Rankings, Maritime and Nickel all give the rank column 200px against a 148px Site column, so every ranked table opens with a wide empty gutter. It is consistent across tabs, which is why it was left alone rather than fixed in one of them — the fix belongs in the shared `.micro-table` rule, and it should be checked against all of them at once.
- **[low] `nickel_acid_mi` is joined but never rendered.** The nickel-anchor join computes distance to bulk sulfuric acid for every site and nothing surfaces it — no detail row, no table column, no CSV, no provenance entry of its own. It is deliberately unscored (large hydromet plants burn sulfur on site and make their own acid, so it is a cost input rather than a siting determinant), but carrying a field no surface reads is the kind of thing that quietly rots. Either add a detail-panel row with a provenance entry, or drop the field from the connector output.
- **[med] Extend `scripts/select_tests.py` coverage as the suite grows.** The impacted-test selector maps changed files to the tests that can observe them and fails open on anything unrecognised. Today's rules cover the score modules, connectors, builders, CSS and prose; every new test file needs a rule or it only ever runs via the full suite. `test_every_rule_points_at_a_test_path_that_exists` catches deleted targets but cannot catch a missing rule.
