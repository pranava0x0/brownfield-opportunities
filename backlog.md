...
- **[high] `In_Reuse` flag quick win (zero new fetches).** The `RedevelopmentAppSitePoints` FeatureServer (which `epa_redev.py` already fetches) carries an `In_Reuse = "Yes"/"No"` field that is never surfaced in the UI or schema. Expose it: add `in_reuse: bool | None` to `SiteRecord`, populate it in `connectors/epa_redev.py`, surface as a green "Active Reuse" pill in the detail panel next to the program pill (same pattern as the "DC candidate" pill). Zero additional HTTP calls; requires only a schema update + two lines in `selectSite()`. Ship this before any of the scrape-based connectors above.
