...
- **[high] Score bonus for large-plant co-location ("Grid Inherit" signal).** We already emit `power_plant_mw` / `power_plant_fuel` / `power_plant_mi` from the HIFLD EIA-860 layer. **Add to `dc-score.js`**: when `power_plant_fuel` contains "Coal" or "Natural Gas" AND `power_plant_mw ≥ 500` AND `power_plant_mi ≤ 1 mi`, award **+8 pts** as a new `grid_inheritance` component under "Power access." Currently these sites score only on proximity distance — the MW/fuel signal is unused. The badge already shows in the DC Candidates tab; the scoring just needs to reflect it. Add to `_DC_SUIT_GROUPS["Power access"]` and ensure `DC_WEIGHT_TABLE` sums to 100 after the reshuffle (take 8 pts from `power_plant` component, rebalance `transmission_distance` by −4 and `substation` by −4). Guard: `test_grid_inheritance_component_weight_table_sums_to_100`.
...
- **[high] EO 14318 eligibility badge.** Flag sites that meet the EPA's stated criteria from the January 2026 guidance: `program ∈ {superfund, brownfield}` AND `acreage ≥ 100` AND `transmission_mi ≤ 2` AND `in_sfha !== true`. Show a `Fed Fast Lane` badge in the DC Candidates Signals column and in the detail panel. No new data needed — computed entirely from existing fields. Add to `computeDcCompositeScore` as a `+3` readiness bonus (distinct from the existing `data_center_reuse_candidate` bonus). Update `READINESS_WEIGHT` component accordingly.
...
- **[high] Rural OZ score bonus in `dc-score.js`.** `oz_rural` is already in the schema and emitted by the `opportunity-zone` connector, but `dc-score.js` treats rural and non-rural OZ identically (both `+5`). Implement a two-tier OZ bonus: `oz_rural === true` → `+7` (vs `+5` for any OZ). The extra 2 points reflect the Rural QROF 30% basis step-up premium vs. standard OZ 15%. Guard: `test_oz_rural_bonus_higher_than_standard_oz`.
