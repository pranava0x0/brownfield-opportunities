...
- **[high] Wire `RAU_Status` (Sitewide Ready for Anticipated Use) through epa-redev.** Already in `OUTFIELDS` and in every cache file (`data/cache/epa-redev_*.json`) — the connector fetches it and drops it in `normalize()`. Source distribution (live-verified 2026-06-10): 488 "Meets the Measure" / 444 "Does Not Meet the Measure" / 43 retracted / 7 formerly-retracted / 18 null. SWRAU is EPA's own answer to "is ALL of this site's land ready for its anticipated use" — the single best public per-site land-availability signal. Zero new HTTP: add `rau_status` to schema + normalize, re-run connector (cache hit), surface as a readiness pill + detail row + DC-score readiness sub-signal.
...
- **[high] Unify the DC Candidates view with the global filter system.** The candidates tab ignores the entire filter strip (search / state / program / acreage / OZ) AND duplicates two of its concepts with unsynced parallel controls: "Min tier" duplicates the persona buttons (`?dc_tier=`), "Readiness: Opp. Zone / EPA candidate" duplicates the OZ checkbox (`?oz=1`) and the DC-candidate KPI toggle (`?dc_candidate=1`). A user filtered to California on the map clicks DC Candidates and silently sees nationwide results. None of the candidates filters are URL-persisted. Options (see session notes): (1) merge candidates into the Table as a "Power signals" column preset + score-desc default sort, delete `candidatesState` + duplicated infinite-scroll (~300 lines); (2) cheaper: make `buildCandidatesView()` read the global `filterState`, delete the duplicate tier/readiness button groups, keep only the DC/Gen lens toggle (URL-persist as `?lens=`).
