...
- **[med] DOM-budget drift — first-paint is ~4,876 nodes, cap is 5,000.** `test_dom_size_under_5k_nodes`'s docstring (and the CLAUDE.md "Total DOM nodes ~2,700" note) are stale: measured first-paint is now ~4,876, mostly from the hidden Rankings + Nuclear-Siting tab tables that render into the DOM even when their tab is inactive. There's <130 nodes of headroom, so any new column/section trips the guard (see the reverted parcel column above). Options: lazy-build the hidden tab tables on first activation (biggest win), or shrink the first table page. Until then, treat the 5k cap as effectively full and update the stale ~2,700 claim in CLAUDE.md + the test docstring.
