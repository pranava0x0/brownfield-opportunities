...
The v1.11.2 commit fixed the ECHO connector and shipped 380 of 1,787 Final/Deleted Superfund sites with enforcement data. Batches 2–5 (2026-05-05/06) added 796 more sites, bringing ECHO coverage to **1,176/1,787 (65.8%)**; subsequent runs completed ECHO at **1,787/1,787 (100%)**. A parallel docs backfill is now at **1,420/1,787 (79.5%)** as of 2026-05-23 after a `--missing-only --docs-skip 24 --docs-limit 10` batch added 10 document-bearing sites. Continued docs coverage is gated by upstream API throttling, so this is staged work, not a single sprint:

- ~~**[high] Continue ECHO backfill.**~~ Done — ECHO coverage is now 1,787/1,787 F/D Superfund sites.
- **[high] Continue docs backfill (367 eligible sites remaining).** Prefer resumable merge mode: `python3 refresh.py --source epa-superfund-docs --missing-only --docs-limit 50`; if the next run keeps rechecking known no-doc misses, use a small bounded skip window such as `--docs-skip 34 --docs-limit 10` and verify the count advances. The 2026-05-23 batches showed a bimodal pattern: early records were mostly cached "no docs / no SF_SITE_ID" misses, then later records were document-rich and slow because EPA's `semspub` cachejson endpoint took ~60s per collection. Keep future batches bounded at 25-50 unless running overnight.
- **[med] Regenerate ai-summary after each docs batch.** Static summaries are cache-aware (`_fingerprint()` hashes the enrichment fields). Run `python3 refresh.py --source ai-summary --ai-static --ai-status F,D --ai-limit 0` after each batch; only sites whose ECHO/docs data changed will be regenerated (~3-5 s total in static mode). Already done: 1,787 sites include enforcement context; 1,420 sites now have document-aware summary context.
