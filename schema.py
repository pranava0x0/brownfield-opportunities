...
class HanfordSourceLink(BaseModel):
    """One labeled citation link on a DOE-site dossier row (Hanford-named for
    identifier continuity; validates every DOE site dossier)."""
...
class DoeInfrastructureRow(BaseModel):
    """One cited site-level infrastructure fact on a DOE-site dossier.

    The seven categories are the dossier's infrastructure vocabulary —
    `power_td` (transmission & distribution / interconnection) and
    `natural_gas` were added 2026-08-24 by user direction alongside the
    classic water/rail/road/fiber/workforce set. Every row is a curated
    claim with its own citation; a category with no verifiable public
    ...
    category: Literal[
        "power_td", "natural_gas", "water", "rail", "road", "fiber", "workforce",
    ]
    summary: str
    source_label: str
...
class HanfordOpportunity(BaseModel):
    ...
    kind: Literal[
        ...
        # Added 2026-08-24 for the SRS/Portsmouth/Paducah/WIPP dossiers:
        "gas_generation", "hydrogen_ammonia", "nuclear_fuel_cycle",
        "waste_disposal",
    ]
...
class HanfordFacilityFit(BaseModel):
    """One row of the data-center-vs-reactor-class comparison on a DOE-site
    ...
    2026-08-24 rework: every cell now names its binding `constraint` (the
    one factor that decides the fit — "mission", "water", "licensing",
    "land", "power", "gas", "workforce", "security") and may carry its own
    `sources` when the rationale asserts a fact beyond the parcel's own
    row-level citation. The rationale must be visible in the UI, never
    tooltip-only.
    """
    ...
    sources: Optional[list[HanfordSourceLink]] = None


class HanfordParcel(BaseModel):
    """One named land unit of a DOE site in an E2E NEPA dossier.

    Hanford-named for identifier continuity, but since 2026-08-24 this class
    validates every DOE-site dossier file (hanford/srs/portsmouth/paducah/
    wipp-e2e.json). 'Parcel' means a named land unit of a federal site —
    never a cadastral parcel; coordinates are representative points, not
    boundaries. Same provenance contract as CoalConversionAsset: every row
    carries `source_url` (verified resolving) + `verified_at` (YYYY-MM-DD
    audit stamp). The screening/geojson/corpus fields are generated by the
    dossier builders, not curated; they are typed loosely here because
    their shape belongs to nepa-mcp and to the corpus join, while
    ...
    kind: Literal[
        ...
        "conservation", "cultural", "context_campus", "operating_mission",
        "leased_industrial",
    ]
    lat: float
    lon: float
    coord_note: str
    approx_acres: Optional[float] = Field(
        default=None, ge=0.0,
        description="None = no published figure exists (rendered as 'size "
        "not published', never invented — e.g. the Centrus ACP footprint).",
    )
